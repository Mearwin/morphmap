...
import type { GameStoreState } from '../store/gameStoreReducer'

export function parseHash(): { game: string | null; tag: string | null; view: 'lineage' | null; embed: boolean; depth: number | null } {
  const hash = window.location.hash.slice(1)
  if (!hash) return { game: null, tag: null, view: null, embed: false, depth: null }
...
export function readInitialStateFromHash(): Partial<GameStoreState> {
  const { game, tag, view, embed, depth } = parseHash()
  return {
